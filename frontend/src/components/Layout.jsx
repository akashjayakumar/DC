...
import {
  Users, Calendar, FileText, Brain, MessageSquare,
...
const nav = [
  ...
  { label: 'Patient Summary', icon: Brain, to: '/summary', section: 'AI Tools', disabled: true },
  { label: 'Ask Records', icon: MessageSquare, to: '/ask', section: 'AI Tools', disabled: true },
  { label: 'Insurance Q&A', icon: ShieldCheck, to: '/insurance', section: 'AI Tools', disabled: true },
      ...
      <aside className="w-56 flex-shrink-0 bg-white border-r border-gray-100 flex flex-col">
        ...
        <nav className="flex-1 py-2 overflow-y-auto scrollbar-hide">
          {sections.map(section => {
            ...
            return (
              <div key={section}>
                <p className="px-4 pt-3 pb-1 text-[10px] uppercase tracking-wider text-gray-400">
                  {section}
                </p>
                {items.map(({ label, icon: Icon, to, disabled }) =>
                  disabled ? (
                    <div
                      key={to}
                      className="flex items-center gap-2.5 px-4 py-2 text-[13px] text-gray-300 cursor-not-allowed"
                    >
                      <Icon size={15} />
                      {label}
                    </div>
        ...
        <div className="px-4 py-3 border-t border-gray-100 flex items-center gap-2">
          <div className="w-7 h-7 rounded-full bg-teal-50 flex items-center justify-center text-[11px] font-medium text-teal-700">
            DR
          </div>
