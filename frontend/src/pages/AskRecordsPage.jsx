...
import { MessageSquare, Send, Search, Bot, User, Paperclip } from 'lucide-react'
import { getPatients, ragQuery, patientFilter } from '../api/patients'
...
const SUGGESTED = [
  ...
  "What was the diagnosis on the last visit?",
  "What is the insurance coverage amount?",
  "Any follow-up appointments scheduled?",
]

export default function AskRecordsPage() {
  ...
  const selectPatient = (p) => {
    ...
    setMessages([{
      role: 'assistant',
      text: `Hi! I can answer questions about ${p.name}'s uploaded records and visit history. What would you like to know?`,
    }])
  }
  ...
  try {
    let res
    if (!selectedPatient) {
      // No patient selected — try patient-filter style query first
      res = await patientFilter(q)
      setMessages(m => [...m, { role: 'assistant', text: res.answer }])
    } else {
      res = await ragQuery(q, selectedPatient._id)
      setMessages(m => [...m, { role: 'assistant', text: res.answer, sources: res.sources }])
    }
  } catch (e) {
    setMessages(m => [...m, { role: 'assistant', text: 'Error reaching the AI. Check your backend.' }])
  } finally {
    setLoading(false)
  }
}
      ...
      <div className="w-64 flex-shrink-0 border-r border-gray-100 bg-white flex flex-col">
        <div className="px-4 py-3 border-b border-gray-100">
          <h1 className="text-[14px] font-medium text-gray-800 flex items-center gap-2">
            <MessageSquare size={15} className="text-brand-600" /> Ask Records
          </h1>
          <p className="text-[11px] text-gray-400 mt-0.5">Ask questions about patient documents</p>
        </div>
        ...
        <div
          onClick={() => {
            ...
            setMessages([{ role: 'assistant', text: 'Hi! Ask me anything about any uploaded dental records or documents.' }])
          }}
          className={clsx(
            'flex items-center gap-2.5 px-3 py-2.5 cursor-pointer border-b border-gray-100 transition-colors',
            !selectedPatient ? 'bg-brand-50 border-l-2 border-brand-600' : 'hover:bg-gray-50'
          )}
        >
          ...
          <div>
            <p className="text-[12px] font-medium text-gray-800">All records</p>
            <p className="text-[10px] text-gray-400">Search across all documents</p>
          </div>
        </div>
          ...
          {filtered.map(p => (
            <div
              ...
              className={clsx(
                ...
                selectedPatient?._id === p._id ? 'bg-brand-50 border-l-2 border-brand-600' : 'hover:bg-gray-50'
              )}
            >
              <Avatar name={p.name} />
              <div className="min-w-0">
                <p className="text-[12px] font-medium text-gray-800 truncate">{p.name}</p>
                <p className="text-[10px] text-gray-400">{p.age} yrs</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="flex-1 flex flex-col bg-gray-50 overflow-hidden">
        ...
        <div className="bg-white border-b border-gray-100 px-5 py-3 flex items-center gap-2">
          {selectedPatient ? (
            <>
              ...
              <div>
                <p className="text-[13px] font-medium text-gray-800">{selectedPatient.name}</p>
                <p className="text-[11px] text-gray-400">Asking about this patient's records</p>
              </div>
            </>
          ) : (
            <>
              ...
              <div>
                <p className="text-[13px] font-medium text-gray-800">All records</p>
                <p className="text-[11px] text-gray-400">Searching across all uploaded documents</p>
              </div>
            </>
          )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide px-5 py-4 space-y-4">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full gap-4 text-gray-300">
              ...
              <p className="text-[13px]">Select a patient or choose "All records" to start asking</p>
            </div>
          )}
                ...
                {msg.sources?.length > 0 && (
                  <div className="mt-2 pt-2 border-t border-gray-100 flex flex-wrap gap-1">
                    {msg.sources.map((s, si) => (
                      <span key={si} className="text-[10px] bg-gray-50 text-gray-500 px-1.5 py-0.5 rounded flex items-center gap-1">
                        <Paperclip size={9} /> {s}
                      </span>
                    ))}
                  </div>
          ...
          {loading && (
            <div className="flex gap-2.5">
              ...
              <div className="bg-white border border-gray-100 rounded-xl rounded-tl-none px-4 py-3">
        ...
        <div className="bg-white border-t border-gray-100 px-4 py-3 flex items-center gap-2">
          <input
            ...
            placeholder={messages.length === 0 ? "Select a patient first..." : "Ask about this patient's records..."}
            value={input}
